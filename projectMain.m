    ...
    for k = 1:num_frames
           ...
           if(max_val > 0.00001)
            %--------------------if val > threshold frame is not silent--------------
               ...
               count=count+1;
               f0 = quantizeFreq(pitch(frame,fs),notes,num_of_notes,tolerance);
...
if(mode == 2)
    raga = input('enter raga to be checked - ');
    [z,fs] = record();
...
function [z,fs] = record()
    rec_time = input('Enter how much time(in seconds) raga is to be recorded-');
    if(rec_time < 3)
        rec_time = 3;
    end
...
%---------------------mapping each f0 to quantized notes-----------------
function [f] = quantizeFreq(frequency,octave_freq, num_of_notes, tolerance)
        f = 0;
        for j = 1 : num_of_notes
           if(abs(frequency - octave_freq(j)) <= tolerance)
                f = octave_freq(j);
                break;
           end
       end
